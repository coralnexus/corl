...
If no resources are found, it returns without creating anything.
    ...
    Coral.run do
      ...
      config = Coral::Config.init_flat(options, [ :resource, :coral_resources ], {
        :hiera_scope     => self,
        :puppet_scope    => self,
        :search          => 'core::default',
      ...
      resources = Coral::Config.normalize(resources, override_var, config)
      defaults  = Coral::Config.normalize(defaults, default_var, config)
          
      Coral.provisioner(:puppet).add(definition_name, resources, defaults, config)
    end
  end
end